...
export default function RegisterBrokerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.phone.trim()) {
      setError('Telefone é obrigatório para corretores');
      setLoading(false);
      return;
    }

    if (!formData.creci.trim()) {
      setError('Número do CRECI é obrigatório');
      setLoading(false);
      return;
    }
    ...
    try {
      const yearsExp = formData.yearsExperience ? parseInt(formData.yearsExperience) : undefined;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[url('/img-1.jpg')] bg-cover bg-center">
      ...
      <main className="relative z-10 w-full max-w-2xl px-6 py-20">
        {/* Logo */}
        ...
        <div className="card-glass p-8 animate-fade-in-up">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label htmlFor="creci" className="block text-sm font-semibold text-white/90 mb-2">
                  CRECI *
                </label>
                <input
                  id="creci"
            ...
            <div>
              <label htmlFor="yearsExperience" className="block text-sm font-semibold text-white/90 mb-2">
                Anos de Experiência
              </label>
              <input
                id="yearsExperience"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Criando conta...' : 'Criar Conta de Corretor'}
            </button>
          </form>
