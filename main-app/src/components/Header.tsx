    ...
    <header className="w-full fixed top-0 left-0 right-0 z-50 backdrop-blur-3xl bg-white/12 border-b border-white/25 shadow-xl">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-4 md:gap-6">
        ...
        <div className="flex-1 hidden md:block">
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center bg-white/10 backdrop-blur-md border border-white/25 rounded-full shadow-lg px-4 py-2.5 hover:bg-white/15 transition-all">
              ...
              <button 
                aria-label="Buscar" 
                className="ml-3 px-5 py-2 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white text-sm font-bold shadow-lg hover:shadow-xl transition-all"
              >
                Buscar
              </button>
            </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 ml-auto">
          <Link 
            href="/imoveis" 
            className="text-xs sm:text-sm font-bold text-white/95 hover:text-white transition-colors drop-shadow-lg hidden sm:inline"
          >
            Imóveis
          </Link>
          ...
          {user ? (
            <>
              {canRegisterProperties && (
                <Link 
                  ...
                  className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 backdrop-blur-xl border border-white/40 text-xs sm:text-sm font-bold text-white hover:from-green-600 hover:to-emerald-600 transition-all shadow-lg"
                >
                  Cadastrar Imóvel
                </Link>
              )}
              <Link 
                href="/profile" 
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/20 backdrop-blur-xl border border-white/40 text-xs sm:text-sm font-bold text-white hover:bg-white/30 transition-all shadow-lg truncate max-w-[120px] sm:max-w-none"
              >
                {user.name ?? user.email}
              </Link>
              <button
                onClick={logout}
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-red-500/20 backdrop-blur-xl border border-red-500/40 text-xs sm:text-sm font-bold text-red-200 hover:bg-red-500/30 transition-all shadow-lg"
              >
                Sair
              </button>
            </>
            ...
            <Link 
              href="/auth/login" 
              className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-white/20 backdrop-blur-xl border border-white/40 text-xs sm:text-sm font-bold text-white hover:bg-white/30 transition-all shadow-lg"
            >
              Entrar
            </Link>
          )}
